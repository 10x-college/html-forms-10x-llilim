...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function launchSpaceship() {
    ...
    setTimeout(() => {
      spaceshipContainer.style.animation =
        "launchToCenter 1.5s cubic-bezier(0.45, 0, 0.55, 1) forwards";
    }, 100);
  ...
  function createSpaceCanvas() {
    ...
    function animateSpace() {
      ...
      stars.forEach((star) => {
        ...
        // Draw star
        ctx.fillStyle = `rgba(${201 + size * 20}, ${184 + size * 20}, ${
          153 + size * 20
        }, ${0.8 + size / 10})`;
        ctx.beginPath();
        ...
        if (animationSpeed > 5) {
          ...
          ctx.strokeStyle = `rgba(201, 184, 153, ${
            0.3 * (1 - star.z / canvas.width)
          })`;
